...
function create(canvas,invalidate){
 ...
 let program,buffer,position,uv,slot,transform,samplers,resources=new Map(),batch=[],count=0,nearest=false,lost=false,uploads=0,textureBytes=0,drawCalls=0;
 ...
 const fragment='precision highp float;varying vec2 v_uv;varying float v_slot;'+Array.from({length:maxTextures},(_,i)=>'uniform sampler2D u_tex'+i+';').join('')+'void main(){'+Array.from({length:maxTextures},(_,i)=>(i?'else ':'')+(i===maxTextures-1?'':'if(v_slot<'+(i+.5).toFixed(1)+')')+'{gl_FragColor=texture2D(u_tex'+i+',v_uv);}').join('')+'}';
 ...
 function init(){
  ...
  samplers=Array.from({length:maxTextures},(_,i)=>gl.getUniformLocation(program,'u_tex'+i));samplers.forEach((s,i)=>gl.uniform1i(s,i));
 ...
 function flush(){if(!count||lost)return;batch.forEach((r,i)=>{gl.activeTexture(gl.TEXTURE0+i);gl.bindTexture(gl.TEXTURE_2D,r.texture);const filter=nearest?gl.NEAREST:gl.LINEAR;gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MIN_FILTER,filter);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MAG_FILTER,filter);});
  ...
  gl.bufferSubData(gl.ARRAY_BUFFER,0,data.subarray(0,count*5));gl.drawArrays(gl.TRIANGLES,0,count);drawCalls++;count=0;batch=[];
 }
