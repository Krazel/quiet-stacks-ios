...
function create(canvas,invalidate){
 ...
 function init(){
  ...
  smoothing=gl.getUniformLocation(program,'u_smoothing');brightness=gl.getUniformLocation(program,'u_brightness');saturation=gl.getUniformLocation(program,'u_saturation');samplers=Array.from({length:maxTextures},(_,i)=>gl.getUniformLocation(program,'u_tex'+i));samplers.forEach((s,i)=>gl.uniform1i(s,i));
