...
function create(canvas,invalidate){
 ...
 let program,buffer,position,uv,slot,transform,smoothing,brightness,samplers,resources=new Map(),batch=[],count=0,nearest=false,light=1,lost=false,uploads=0,textureBytes=0,drawCalls=0;
 ...
 function init(){
  ...
  smoothing=gl.getUniformLocation(program,'u_smoothing');brightness=gl.getUniformLocation(program,'u_brightness');samplers=Array.from({length:maxTextures},(_,i)=>gl.getUniformLocation(program,'u_tex'+i));samplers.forEach((s,i)=>gl.uniform1i(s,i));
 ...
 function flush(){if(!count||lost)return;batch.forEach((r,i)=>{gl.activeTexture(gl.TEXTURE0+i);gl.bindTexture(gl.TEXTURE_2D,r.texture);const filter=nearest?gl.NEAREST:gl.LINEAR;gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MIN_FILTER,filter);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MAG_FILTER,filter);});
  ...
  gl.uniform1f(smoothing,nearest?0:1);gl.uniform1f(brightness,light);gl.bufferSubData(gl.ARRAY_BUFFER,0,data.subarray(0,count*5));gl.drawArrays(gl.TRIANGLES,0,count);drawCalls++;count=0;batch=[];
 }
 ...
 function draw(image,source,rect,sharp=false,cut=0,gain=1){if(lost)return;const r=resource(image);let index=batch.indexOf(r);if(count&&(nearest!==sharp||light!==gain||(index<0&&batch.length===maxTextures)||count+30>capacity)){flush();index=-1;}nearest=sharp;light=gain;if(index<0){index=batch.length;batch.push(r);}
