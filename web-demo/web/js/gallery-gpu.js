...
function create(canvas,invalidate){
 ...
 let program,buffer,position,uv,slot,transform,smoothing,brightness,saturation,opacity,samplers,resources=new Map(),batch=[],count=0,nearest=false,minified=false,light=1,chroma=1,alpha=1,lost=false,uploads=0,textureBytes=0,drawCalls=0;
 ...
 const fragment=(derivatives?'#extension GL_OES_standard_derivatives : enable\n':'')+'precision highp float;varying vec2 v_uv;varying float v_slot;uniform float u_smoothing;uniform float u_brightness;uniform float u_saturation;uniform float u_opacity;'+Array.from({length:maxTextures},(_,i)=>'uniform sampler2D u_tex'+i+';').join('')+sample+'void main(){'+(derivatives?'vec2 dx=dFdx(v_uv)*.25,dy=dFdy(v_uv)*.25;':'')+Array.from({length:maxTextures},(_,i)=>(i?'else ':'')+(i===maxTextures-1?'':'if(v_slot<'+(i+.5).toFixed(1)+')')+'{gl_FragColor='+(derivatives?'sampleBook(u_tex'+i+',dx,dy)':'texture2D(u_tex'+i+',v_uv)')+';}').join('')+'if(u_brightness!=1.0||u_saturation!=1.0){float grey=dot(gl_FragColor.rgb,vec3(.2126,.7152,.0722));gl_FragColor.rgb=clamp(mix(vec3(grey),gl_FragColor.rgb,u_saturation)*u_brightness,vec3(0.),vec3(gl_FragColor.a));}gl_FragColor*=u_opacity;}';
 ...
 function init(){
  ...
  smoothing=gl.getUniformLocation(program,'u_smoothing');brightness=gl.getUniformLocation(program,'u_brightness');saturation=gl.getUniformLocation(program,'u_saturation');opacity=gl.getUniformLocation(program,'u_opacity');samplers=Array.from({length:maxTextures},(_,i)=>gl.getUniformLocation(program,'u_tex'+i));samplers.forEach((s,i)=>gl.uniform1i(s,i));
 ...
 function flush(){if(!count||lost)return;batch.forEach((r,i)=>{gl.activeTexture(gl.TEXTURE0+i);gl.bindTexture(gl.TEXTURE_2D,r.texture);const filter=nearest?gl.NEAREST:gl.LINEAR;gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MIN_FILTER,filter);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MAG_FILTER,filter);});
  ...
  gl.uniform1f(smoothing,!nearest&&minified?1:0);gl.uniform1f(brightness,light);gl.uniform1f(saturation,chroma);gl.uniform1f(opacity,alpha);gl.bufferSubData(gl.ARRAY_BUFFER,0,data.subarray(0,count*5));gl.drawArrays(gl.TRIANGLES,0,count);drawCalls++;count=0;batch=[];
 }
 ...
 function draw(image,source,rect,sharp=false,cut=0,gain=1,vivid=1,transparency=1){if(lost)return;const r=resource(image);let index=batch.indexOf(r);if(count&&(nearest!==sharp||light!==gain||chroma!==vivid||alpha!==transparency||(index<0&&batch.length===maxTextures)||count+30>capacity)){flush();index=-1;}nearest=sharp;light=gain;chroma=vivid;alpha=transparency;if(index<0){index=batch.length;batch.push(r);}
