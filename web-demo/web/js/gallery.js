(() => {
  ...
  function endPointer(e,cancel=false){if(autoTesting||!pointers.has(e.pointerId))return;profiler?.input('pointerup');const p=world(local(e)),g=gesture;pointers.delete(e.pointerId);if(!cancel&&!suppressTap&&g){if(g.moved&&g.id!==undefined){const dropStart=profiler?.active?performance.now():null;const dropped=model.drop(g.id,p,safeBottom?target=>visibleDrop(g.id,target):undefined,safeBottom?dragPosition(model.book(g.id),p):undefined);if(dropStart!==null)profiler.drop(performance.now()-dropStart);if(dropped){feedback?.play(model.placement(g.id),pointFor(model.book(g.id)));selected=g.id;series=model.book(g.id).series;save();update();}}else if(!g.moved)tap(p);}gesture=null;dragPoint=null;pinch=null;canvas.classList.remove('grabbing');if(!pointers.size)suppressTap=false;else suppressTap=true;constrain();save();update();}
  ...
  for(const [id,mode] of [['demo-sort','sort'],['demo-scatter','scatter']])$(id).onclick=()=>{if(!ready||autoTesting||pointers.size)return;model.demoArrange(mode);feedback?.clear();selected=null;dragPoint=null;$('inspection').hidden=true;$('demo-actions').hidden=true;$('demo-toggle').setAttribute('aria-expanded','false');save();update();};
