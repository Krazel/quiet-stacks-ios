(() => {
  ...
  const imageError=()=>{$('loading').querySelector('h2').textContent='The gallery could not load.';$('loading').querySelector('p').textContent='Please try loading the room again.';$('retry').hidden=false;};
  function loadAssets(){ready=false;loadedImages=0;background.onload=imageLoaded;atlasImage.onload=()=>{try{atlas=GalleryTextures.importAtlas(atlasImage);imageLoaded();}catch{imageError();}};floorImage.onload=()=>{try{floorAtlas=GalleryTextures.importAtlas(floorImage);imageLoaded();}catch{imageError();}};directionsImage.onload=()=>{try{directionsAtlas=GalleryTextures.importAtlas(directionsImage);imageLoaded();}catch{imageError();}};background.onerror=imageError;atlasImage.onerror=imageError;floorImage.onerror=imageError;directionsImage.onerror=imageError;turnImage.onload=()=>{try{turnAtlas=GalleryTextures.importAtlas(turnImage);imageLoaded();}catch{imageError();}};turnImage.onerror=imageError;bindingImages.forEach((img,i)=>{img.onload=()=>{try{bindingAtlases[i]=GalleryTextures.importAtlas(img);imageLoaded();}catch{imageError();}};img.onerror=imageError;});openImage.onload=()=>{try{openAtlas=GalleryTextures.importAtlas(openImage);imageLoaded();}catch{imageError();}};openImage.onerror=imageError;repairImage.onload=imageLoaded;repairImage.onerror=imageError;collectionImages.forEach((img,i)=>{img.onload=()=>{try{collectionAtlases[i]=GalleryTextures.importAtlas(img,true);imageLoaded();}catch{imageError();}};img.onerror=imageError;});nameplateImage.onload=imageLoaded;nameplateImage.onerror=imageError;volumeImages.forEach((img,i)=>{img.onload=()=>{try{volumeAtlases[i]=GalleryTextures.importAtlas(img,true);imageLoaded();}catch{imageError();}};img.onerror=imageError;});const suffix='?v=16';volumeImages.forEach((img,i)=>{img.src=GalleryVolumes.atlases[i].file+suffix;});nameplateImage.src='assets/nameplates-v14.png'+suffix;repairImage.src='assets/gallery-repair-v13.png'+suffix;collectionImages.forEach((img,i)=>{img.src=GalleryModel.COLLECTION_ATLASES[i].file+suffix;});openImage.src='assets/books-open-v12.png'+suffix;background.src='assets/gallery-empty.png'+suffix;atlasImage.src='assets/books-varied-v9.png'+suffix;floorImage.src='assets/books-floor-v6.png'+suffix;directionsImage.src='assets/books-directions-v7.png'+suffix;turnImage.src='assets/books-turn-v8.png'+suffix;bindingImages.forEach((img,i)=>{img.src='assets/bindings-'+['b','c','d'][i]+'-v9.png'+suffix;});}
