(() => {
  ...
  const nameplateImage=new Image();let room;const collectionImages=GalleryModel.COLLECTION_ATLASES.map(()=>new Image()),collectionAtlases=[...collectionImages];
  const roomArt=window.GalleryRoomArtwork,roomTiles=roomArt?.tiles.map(tile=>({...tile,image:new Image()}));
  ...
  function originalBookVisual(b,view){if(view===1){const v=GalleryVolumes.bindings[SERIES[b.series].art]?.[b.volume-1];if(v)return {image:volumeAtlases[v.atlas],source:v.source};}const art=SERIES[b.series].art,v=floorVariant(b);if(art>=100){const group=13+Math.floor((art-100)/6),sheet=GalleryModel.COLLECTION_ATLASES[group],col=(art-100)%6,row=view===0?v:view===1?2:3;return {image:collectionAtlases[group],source:sheet.sprites[row*6+col]};}if(view===0&&v===3)return {image:openAtlas,source:OPEN_SPRITES[art]};if(art>=24){const group=Math.floor((art-24)/6),sheet=GalleryModel.COLLECTION_ATLASES[group],col=(art-24)%6,row=view===0?v:view===1?2:3;return {image:collectionAtlases[group],source:sheet.sprites[row*sheet.columns+col]};}if(art>=6){const group=Math.floor(art/6)-1,col=art%6,row=view===0?v:view===1?2:3;return {image:bindingAtlases[group],source:BINDING_SPRITES[group][row*6+col]};}if(view!==0)return {image:atlas,source:SPRITES[(view-1)*6+art]};return v===2?{image:turnAtlas,source:TURN_SPRITES[art]}:v?{image:directionsAtlas,source:DIRECTION_SPRITES[art*3+v-1]}:{image:floorAtlas,source:FLOOR_SPRITES[art]};}
  function bookVisual(b,view){if(view===1&&!GalleryVolumes.bindings[SERIES[b.series].art]?.[b.volume-1])throw new Error('Missing painted volume artwork');const key=b.id*3+view;if(visualCache.has(key))return visualCache.get(key);const visual=originalBookVisual(b,view);if(!packed){visualCache.set(key,visual);return visual;}
  ...
  function dragPosition(b,p){if(!safeBottom)return p;const r=bookRect(b,p,true),a=world({x:4,y:4}),z=world({x:width-4,y:height-safeBottom-6});return {x:clamp(p.x,a.x+r.w/2,Math.max(a.x+r.w/2,z.x-r.w/2)),y:clamp(p.y,a.y+r.h,Math.max(a.y+r.h,z.y))};}
  ...
  // Preview and release share the same geometric resolver, including safe-area fallback.
  function resolveDrop(id,p,commit=false){
    const accept=safeBottom?target=>visibleDrop(id,target):undefined;
    const seed=safeBottom?dragPosition(model.book(id),p):undefined;
    return commit?model.drop(id,p,accept,seed):model.nearestDrop(id,p,accept,seed);
  }
  function dropPreview(){
    const g=gesture,p=g?.last;
    if(!g?.moved||g.id===undefined||!dragPoint||pinch||suppressTap||!p||p.x<0||p.x>width||p.y<0||p.y>height-safeBottom)return null;
    const target=resolveDrop(g.id,world(p));if(!target||target.place==='floor')return null;
    const b={...model.book(g.id),place:target.place,slot:target.slot??-1,cartSlot:target.place==='cart'?target.slot:undefined};
    const point=target.place==='shelf'?slotPoint(target.slot):target.place==='cart'?GalleryModel.cartPoint(target.slot):target.point;
    return {id:g.id,place:target.place,slot:target.slot??-1,point,rect:bookRect(b,point)};
  }
  function cancelDrag(){
    if(gesture?.id!==undefined)bookMotion?.release(gesture.id,true);
    gesture=null;dragPoint=null;pinch=null;suppressTap=false;pointers.clear();canvas.classList.remove('grabbing');requestDraw();
  }
  ...
  function hitBook(p){if(safeBottom&&screen(p).y>height-safeBottom-6)return;let nearest,dist=Infinity;const books=orderedBooks().slice().reverse(),margin=Math.min(18,12/scale());
  ...
  canvas.addEventListener('pointerdown',e=>{if(!ready||autoTesting||story?.active||e.button>0)return;profiler?.input('pointerdown');canvas.setPointerCapture(e.pointerId);const p=local(e);pointers.set(e.pointerId,p);if(pointers.size===2){pinchStart();return;}if(pointers.size>2)return;const b=hitBook(world(p));gesture={start:p,last:p,id:b?.id,moved:false};canvas.classList.add('grabbing');});
  canvas.addEventListener('pointermove',e=>{if(autoTesting||!pointers.has(e.pointerId))return;profiler?.input('pointermove');requestDraw();const p=local(e);pointers.set(e.pointerId,p);if(pointers.size>=2&&pinch){const [a,b]=[...pointers.values()],mid={x:(a.x+b.x)/2,y:(a.y+b.y)/2};camera().zoom=clamp(pinch.zoom*Math.hypot(a.x-b.x,a.y-b.y)/pinch.distance,minZoom(),8);camera().x=pinch.anchor.x-(mid.x-width/2)/scale();camera().y=pinch.anchor.y-(mid.y-height/2)/scale();constrain();return;}if(suppressTap||!gesture)return;const g=gesture;if(!g.moved&&Math.hypot(p.x-g.start.x,p.y-g.start.y)>7){g.moved=true;if(g.id!==undefined){$('inspection').hidden=true;select(g.id,false);bookMotion?.lift(g.id);window.GalleryAudio?.effect('pickup');}}if(g.moved){if(g.id!==undefined){$('inspection').hidden=true;selected=g.id;series=model.book(g.id).series;dragPoint=dragPosition(model.book(g.id),world(p));}else{camera().x-=(p.x-g.last.x)/scale();camera().y-=(p.y-g.last.y)/scale();constrain();}}g.last=p;});
  function endPointer(e,cancel=false){if(autoTesting||!pointers.has(e.pointerId))return;profiler?.input('pointerup');const p=world(local(e)),g=gesture;pointers.delete(e.pointerId);if(!cancel&&!suppressTap&&g){if(g.moved&&g.id!==undefined){const dropStart=profiler?.active?performance.now():null;const wasExact=model.placement(g.id)==='exact';const dropped=resolveDrop(g.id,p,true);if(dropStart!==null)profiler.drop(performance.now()-dropStart);bookMotion?.release(g.id,!dropped);if(dropped){window.GalleryAudio?.effect(model.book(g.id).place==='floor'?'floor':'place');if(!wasExact&&model.placement(g.id)==='exact')window.GalleryAudio?.effect('correct');story?.check();feedback?.play(model.placement(g.id),pointFor(model.book(g.id)));selected=g.id;series=model.book(g.id).series;save();update();}}else if(!g.moved)tap(p);}if(cancel&&g?.id!==undefined)bookMotion?.release(g.id,true);gesture=null;dragPoint=null;pinch=null;canvas.classList.remove('grabbing');if(!pointers.size)suppressTap=false;else suppressTap=true;constrain();save();update();}
  ...
  canvas.addEventListener('wheel',e=>{e.preventDefault();if(autoTesting||story?.active)return;profiler?.input('wheel');zoom(Math.exp(-e.deltaY*.0015),local(e));},{passive:false});
  ...
  for(const [id,mode] of [['demo-sort','sort'],['demo-scatter','scatter']])$(id).onclick=()=>{if(!ready||autoTesting||pointers.size)return;story?.cancelPending();model.demoArrange(mode);feedback?.clear();bookMotion?.clear();selected=null;dragPoint=null;$('inspection').hidden=true;$('demo-actions').hidden=true;$('demo-toggle').setAttribute('aria-expanded','false');save();update();};
  window.addEventListener('keydown',e=>{if(autoTesting||story?.active||/INPUT|SELECT|TEXTAREA/.test(e.target.tagName))return;profiler?.input('keydown');let handled=true;switch(e.key){case 'Home':camera().zoom=1;camera().x=W/2;camera().y=H/2;break;case '+':case '=':zoom(1.2);break;case '-':zoom(1/1.2);break;case 'ArrowLeft':camera().x-=65/scale();break;case 'ArrowRight':camera().x+=65/scale();break;case 'ArrowUp':camera().y-=65/scale();break;case 'ArrowDown':camera().y+=65/scale();break;case 'Escape':cancelDrag();$('demo-actions').hidden=true;$('demo-toggle').setAttribute('aria-expanded','false');$('inspection').hidden=true;selected=null;update();break;default:handled=false;}if(handled){e.preventDefault();constrain();save();}});
  function showInspection(b){
    window.GalleryAudio?.effect('inspect');
  ...
  let visibleBounds,litVisual,litCanvas;
  function illuminated(visual){
    if(litVisual===visual)return litCanvas;const [x,y,w,h]=visual.source,c=document.createElement('canvas');c.width=w;c.height=h;const ic=c.getContext('2d');ic.drawImage(visual.image,x,y,w,h,0,0,w,h);
    // Source-atop retains the sprite alpha, including iOS 16 without Canvas filters.
    ic.globalCompositeOperation='source-atop';ic.fillStyle='rgba(255,246,224,.25)';ic.fillRect(0,0,w,h);litVisual=visual;litCanvas=c;return c;
  }
  function bookArt(b,p,held=false){const baseRect=bookRect(b,p,held),r=bookMotion?.rect(b.id,baseRect,scale())||baseRect;
    if(r.x+r.w<visibleBounds.left||r.x>visibleBounds.right||r.y+r.h<visibleBounds.top||r.y>visibleBounds.bottom)return;
    if(perfMeasuring)perfBookCount++;const visual=bookVisual(b,r.view),source=visual.source;/* Preserve the painted ground plane and native aspect ratio. */const light=b.id===selected?1.4:1,chroma=b.id===selected?1.35:1;if(gpu)gpu.draw(visual.image,source,r,false,0,light,chroma);else{const lit=light>1?illuminated(visual):null;if(lit)ctx.drawImage(lit,0,0,lit.width,lit.height,r.x,r.y,r.w,r.h);else ctx.drawImage(visual.image,...source,r.x,r.y,r.w,r.h);}

  }
  function frame(time){framePending=false;if(!ready)return;const motionActive=bookMotion?.tick();perfMeasuring=!!profiler?.active;perfBookCount=0;const perfStart=perfMeasuring?performance.now():0;const s=scale(),c=camera(),held=model.book(selected);
    if(gpu){if(!gpu.begin(width,height,dpr,s,c))return;if(roomTiles){for(const tile of roomTiles)gpu.draw(tile.image,tile.source,tile.rect,false);}else gpu.draw(room,[0,0,W+320,H],{x:-130,y:0,w:W+320,h:H},true);if(!roomArt?.embeddedTitles)for(const plate of GalleryRoom.NAMEPLATES){const [cx,cy]=plate.center,w=plate.width,h=w*plate.source[3]/plate.source[2];gpu.draw(nameplateImage,plate.source,{x:cx-w/2,y:cy-h/2,w,h},false,2.6);}}
    else{ctx.setTransform(dpr,0,0,dpr,0,0);ctx.fillStyle='#17120e';ctx.fillRect(0,0,width,height);ctx.translate(width/2-c.x*s,height/2-c.y*s);ctx.scale(s,s);ctx.imageSmoothingEnabled=false;if(roomTiles){ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality='high';for(const tile of roomTiles){const r=tile.rect;ctx.drawImage(tile.image,...tile.source,r.x,r.y,r.w,r.h);}}else ctx.drawImage(room,-130,0,W+320,H);if(!roomArt?.embeddedTitles)GalleryRoom.drawNameplates(ctx,nameplateImage);ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality='high';}const perfRoomEnd=perfMeasuring?performance.now():0;
    ...
    if(preview){const r=preview.rect,v=bookVisual(model.book(preview.id),r.view);
      if(gpu)gpu.draw(v.image,v.source,r,false,0,1,1,.46);
      else{ctx.save();ctx.globalAlpha=.46;ctx.drawImage(v.image,...v.source,r.x,r.y,r.w,r.h);ctx.restore();}
    }
    gpu?.end();feedback?.sync(width,height,s,c);window.__galleryInteraction={dropPreview:preview,selectionGain:held?1.4:1,selectionSaturation:held?1.35:1,bottomClearance:safeBottom?safeBottom+6:0,extraRight:130*s,extraLeft:190*s,selectionOutline:false};ambience?.sync(width,height,scale(),camera());window.__galleryGraphics=gpu?gpu.stats():{backend:'canvas2d'};
  ...
  function loadAssets(){
    ...
    expectedImages=(packed?packed.pages.length+2:10+collectionImages.length+volumeImages.length)+(roomTiles?roomTiles.length-2:0);
    const pending=[];
    ...
    if(roomTiles){for(const tile of roomTiles)watch(tile.image,tile.file);}else watch(background,'assets/gallery-walls-v174.png');
  ...
  if(window.GalleryPerformance){
    ...
    profiler=GalleryPerformance.attach({
      ...
      environment:()=>({viewport:{width,height,dpr},canvas:{width:canvas.width,height:canvas.height},graphics:window.__galleryGraphics,zoom:camera().zoom,totalBooks:TOTAL,ambience:ambience?.stats(),
        ...
        decodedImageEstimateMiB:Math.round([...packedImages,background,nameplateImage].reduce((n,i)=>n+(i.naturalWidth||0)*(i.naturalHeight||0)*4,0)/1048576*100)/100,userAgent:navigator.userAgent||'unknown'}),
      ...
      dropCase(name){if(!dropModel)dropModel=new Gallery();dropModel.demoArrange('sort');if(name==='full-trolley')for(let id=0;id<CART_CAPACITY;id++)dropModel.move(id,'cart');
        const point={'clear-floor':{x:836,y:350},table:{x:830,y:646},lamp:{x:496,y:816},'shelf-left':{x:420,y:460},'shelf-right':{x:1200,y:500},'map-corner':{x:0,y:0},'full-trolley':{x:843,y:505}}[name];
  ...
  if(window.ResizeObserver)new ResizeObserver(resize).observe(canvas);window.addEventListener('pageshow',requestDraw);window.addEventListener('resize',resize);window.addEventListener('gallery-background',cancelDrag);window.addEventListener('pagehide',()=>{cancelDrag();try{localStorage.setItem(KEY,JSON.stringify(model.state));}catch{}});resize();update();requestDraw();
})();
