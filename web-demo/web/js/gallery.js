(() => {
  ...
  function bookVisual(b,view){if(view===1&&!GalleryVolumes.bindings[SERIES[b.series].art]?.[b.volume-1]&&numberedBindings?.bySeries.has(b.series))return numberedBindings.bySeries.get(b.series);const key=b.id*3+view;if(visualCache.has(key))return visualCache.get(key);const visual=originalBookVisual(b,view);if(!packed){visualCache.set(key,visual);return visual;}
  ...
  function constrain(){requestDraw();const c=camera(),s=scale(),hx=Math.min(W/2,width/(2*s)),hy=Math.min(H/2,height/(2*s));c.x=clamp(c.x,hx-130,W+190-hx);c.y=clamp(c.y,hy,H-hy);}
  ...
  canvas.addEventListener('pointermove',e=>{if(autoTesting||!pointers.has(e.pointerId))return;profiler?.input('pointermove');requestDraw();const p=local(e);pointers.set(e.pointerId,p);if(pointers.size>=2&&pinch){const [a,b]=[...pointers.values()],mid={x:(a.x+b.x)/2,y:(a.y+b.y)/2};camera().zoom=clamp(pinch.zoom*Math.hypot(a.x-b.x,a.y-b.y)/pinch.distance,1,8);camera().x=pinch.anchor.x-(mid.x-width/2)/scale();camera().y=pinch.anchor.y-(mid.y-height/2)/scale();constrain();return;}if(suppressTap||!gesture)return;const g=gesture;if(!g.moved&&Math.hypot(p.x-g.start.x,p.y-g.start.y)>7){g.moved=true;if(g.id!==undefined){$('inspection').hidden=true;select(g.id,false);}}if(g.moved){if(g.id!==undefined){$('inspection').hidden=true;selected=g.id;series=model.book(g.id).series;dragPoint=dragPosition(model.book(g.id),world(p));}else{camera().x-=(p.x-g.last.x)/scale();camera().y-=(p.y-g.last.y)/scale();constrain();}}g.last=p;});
  function endPointer(e,cancel=false){if(autoTesting||!pointers.has(e.pointerId))return;profiler?.input('pointerup');const p=world(local(e)),g=gesture;pointers.delete(e.pointerId);if(!cancel&&!suppressTap&&g){if(g.moved&&g.id!==undefined){const dropStart=profiler?.active?performance.now():null;const dropped=model.drop(g.id,p,safeBottom?target=>visibleDrop(g.id,target):undefined,safeBottom?dragPosition(model.book(g.id),p):undefined);if(dropStart!==null)profiler.drop(performance.now()-dropStart);if(dropped){story?.check();feedback?.play(model.placement(g.id),pointFor(model.book(g.id)));selected=g.id;series=model.book(g.id).series;save();update();}}else if(!g.moved)tap(p);}gesture=null;dragPoint=null;pinch=null;canvas.classList.remove('grabbing');if(!pointers.size)suppressTap=false;else suppressTap=true;constrain();save();update();}
  ...
  for(const [id,mode] of [['demo-sort','sort'],['demo-scatter','scatter']])$(id).onclick=()=>{if(!ready||autoTesting||pointers.size)return;story?.cancelPending();model.demoArrange(mode);feedback?.clear();selected=null;dragPoint=null;$('inspection').hidden=true;$('demo-actions').hidden=true;$('demo-toggle').setAttribute('aria-expanded','false');save();update();};
  ...
  function bookArt(b,p,held=false){const r=bookRect(b,p,held);
    ...
    if(r.view===1&&!GalleryVolumes.bindings[SERIES[b.series].art]?.[b.volume-1]){const src=[(b.volume-1)*48,0,48,64],label={x:r.x+r.w*.125,y:r.y+r.h*.65,w:r.w*.75,h:r.h*(1/3)};if(gpu)gpu.draw(marks,src,label,false,0,light,chroma);else ctx.drawImage(marks,...src,label.x,label.y,label.w,label.h);}
