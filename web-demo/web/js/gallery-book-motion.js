...
function create({now=()=>performance.now(),reduced=()=>matchMedia('(prefers-reduced-motion: reduce)').matches}={}){
 const active=new Map();let held=null,time=0,animating=false;
 ...
 function tick(){time=now();animating=false;if(reduced()){active.clear();return false;}for(const [id,e]of active){if(time-e.start>=190)active.delete(id);else animating=true;}if(held&&time-held.start<130)animating=true;return animating;}
