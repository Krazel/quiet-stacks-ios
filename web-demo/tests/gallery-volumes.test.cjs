...
const {LEGACY_SERIES:SERIES}=require('../web/js/gallery-model.js'),{atlases,bindings}=require('../web/js/gallery-volumes.js');
test('all 525 volumes have distinct complete numbered spines within real atlas dimensions',()=>{
 ...
 assert.equal(used.size,525);assert.equal(atlases[bindings[0][0].atlas].file,'assets/moon-volumes-v15.png');
});
