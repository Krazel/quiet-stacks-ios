...
test('all 525 existing identities, shelf assignments, trolley and camera survive the expanded catalog',()=>{
 ...
 const before=structuredClone(input),g=new m.Gallery();assert.ok(g.restore(input));assert.deepEqual(input,before);assert.equal(g.state.books.length,1119);assert.deepEqual(g.state.camera,input.camera);
...
test('every painted collection has a tight non-overlapping run of uniform spines',()=>{
 const used=new Set();for(const c of m.SERIES){assert.equal(c.slotIds.length,c.count);assert.ok(Math.abs(c.slotWidth-c.bookWidth-.18)<1e-8);let right=c.left;for(const [index,id] of c.slotIds.entries()){assert.ok(!used.has(id));used.add(id);const s=m.SLOTS[id],half=c.bookWidth/2;assert.ok(s.x-half>=right-1e-8);if(index)assert.ok(Math.abs(s.x-half-right-.18)<1e-8);assert.ok(s.x+half<=c.right+1e-8);assert.equal(s.y,c.y);right=s.x+half;assert.ok(s.y-c.bookHeight>=c.top);}assert.ok(Math.abs((m.SLOTS[c.slotIds[0]].x-c.bookWidth/2-c.left)-(c.right-right))<1e-8);assert.ok(Math.abs(c.right-right-1)<1e-8);}
