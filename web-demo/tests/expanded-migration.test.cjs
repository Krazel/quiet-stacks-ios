...
test('every painted collection has a tight non-overlapping run of uniform spines',()=>{
 const used=new Set();for(const c of m.SERIES){assert.equal(c.slotIds.length,c.count);assert.ok(c.slotWidth>=9&&c.slotWidth<=11);let right=c.left;for(const id of c.slotIds){assert.ok(!used.has(id));used.add(id);const s=m.SLOTS[id];assert.ok(s.x-4.5>=right-1e-8);assert.ok(s.x+4.5<=c.right+1e-8);assert.equal(s.y,c.y);right=s.x+4.5;assert.ok(s.y-27>=m.RACKS[c.rack].box[1]);}assert.ok(c.right-right<1.1);}
