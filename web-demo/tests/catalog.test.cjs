...
test('every physical bay has a coherent collection and all catalog titles are distinct',()=>{assert.equal(new Set(SLOTS.map(s=>details(s.id).title)).size,TOTAL);for(const collection of SERIES){const members=collection.slotIds.map(id=>SLOTS[id]);assert.ok(members.every(s=>s.series===members[0].series&&s.segment===members[0].segment));assert.ok(collection.category&&collection.name);assert.ok(members.every(s=>details(s.id).category===collection.category));}});
...
test('a version 2 save keeps placed identities while replacing the old floor piles',()=>{const old=JSON.parse(JSON.stringify(require('./fixtures/gallery-v4-catalog.json').save));old.books[0].place='shelf';old.books[0].slot=0;old.version=2;old.books.forEach(b=>{b.series=0;b.volume=1;if(b.place==='floor'){b.x=800;b.y=400;}});const next=new Gallery();assert.ok(next.restore(old));assert.equal(next.book(0).slot,0);assert.equal(next.state.books.filter(b=>b.place==='shelf').length,1);assert.ok(new Set(next.state.books.filter(b=>b.place==='floor').map(b=>b.x)).size>500);});
...
test('each expanded collection has a unique binding, repeated only within its volumes',()=>{
 const {BINDINGS,RACKS}=require('../web/js/gallery-model.js');
 assert.equal(BINDINGS.length,SERIES.length);assert.equal(new Set(SERIES.map(s=>s.art)).size,SERIES.length);
