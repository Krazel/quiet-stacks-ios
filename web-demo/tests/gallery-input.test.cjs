...
function harness(width=1440,height=810,initial,importAtlas=image=>image,usePacked=false){
  ...
  const state=()=>JSON.parse(JSON.stringify(live.state)),screen=([x,y])=>{const c=live.state.camera,s=Math.max(width/1672,height/941)*c.zoom;return [(x-c.x)*s+width/2,(y-c.y)*s+height/2];};
...
test('all 1119 shelf books keep the original proportions and each collection fits its measured bay',()=>{
  ...
  for(const collection of SERIES){
    ...
    for(const d of images){assert.deepEqual(d.slice(7),images[0].slice(7));}
...
test('drag accepts both ends of a collection bay without an outline',()=>{
  const {SERIES}=require('../web/js/gallery-model.js');
  for(const width of [1440,390])for(const edge of ['left','right']){
    const t=harness(width,844),collection=SERIES.find(s=>s.count>=5&&s.right-s.left>26),b=t.state().books.at(-1);
    ...
    assert.equal(t.state().books[b.id].slot,edge==='left'?collection.slotIds[0]:collection.slotIds.at(-1));
