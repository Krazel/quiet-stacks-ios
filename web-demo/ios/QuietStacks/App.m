...
#if TARGET_OS_SIMULATOR
        if([NSProcessInfo.processInfo.arguments containsObject:@"--gallery-ui-smoke"])[self.webView evaluateJavaScript:@"setTimeout(()=>{const c=document.getElementById('scene');window.dispatchEvent(new KeyboardEvent('keydown',{key:'Home'}));let s;try{s=JSON.parse(localStorage.getItem('quiet-stacks.gallery.v4'));}catch{}s=s||new GalleryModel.Gallery().state;const b=s.books.filter(b=>b.place==='floor').sort((a,b)=>b.order-a.order)[0];const r=c.getBoundingClientRect(),scale=Math.min(r.width/1672,r.height/941);c.setPointerCapture=()=>{};for(const type of ['pointerdown','pointerup'])c.dispatchEvent(new PointerEvent(type,{pointerId:73,button:0,clientX:r.x+r.width/2+(b.x-836)*scale,clientY:r.y+r.height/2+(b.y-10-470.5)*scale}));const box=id=>{const a=document.getElementById(id).getBoundingClientRect();return {x:a.x,y:a.y,width:a.width,height:a.height,bottom:a.bottom};};const send=view=>webkit.messageHandlers.galleryStatus.postMessage({type:'ui-probe',view,width:innerWidth,height:innerHeight,removed:!document.querySelector('.brand,.map-panel,#home,#plus,#minus'),inspection:box('inspection'),close:box('inspect-close'),action:box('inspect-pickup'),summary:box('selection'),title:document.getElementById('inspect-title').textContent,summaryHidden:document.getElementById('selection').hidden});send('details');setTimeout(()=>{document.getElementById('inspect-close').click();send('summary');},7000);},700);" completionHandler:nil];
