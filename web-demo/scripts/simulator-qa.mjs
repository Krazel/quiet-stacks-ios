...
async function checkBookUI(device,label,boot=false){
 ...
 for(const view of ['details','summary']){let info;for(let n=0;n<60;n++){await new Promise(r=>setTimeout(r,500));try{info=JSON.parse(fs.readFileSync(path.join(folder,'Documents','gallery-ui-'+view+'.json')));break;}catch{}}
  assert.ok(info?.removed);assert.equal(info.fillsView,true);const safe=info.nativeSafeArea;assert.ok(info.scene.x>=safe.left&&info.scene.y>=safe.top);assert.ok(info.scene.x+info.scene.width<=info.width-safe.right+1);assert.ok(info.scene.bottom<=info.height-safe.bottom-Math.min(12,safe.bottom)+1);if(view==='summary')assert.equal(info.focusVisible,true);assert.equal(info.selectionDisabled,true);assert.equal(info.pageZoom,1);assert.equal(info.minPageZoom,1);assert.equal(info.maxPageZoom,1);assert.ok(info.title);if(view==='details'){assert.ok(info.inspection.height>=250);assert.ok(info.inspection.bottom<=info.height);assert.ok(info.close.height>=44);assert.ok(info.action.bottom<=info.height);assert.ok(info.summaryHidden);}else{assert.ok(!info.summaryHidden);assert.ok(info.summary.bottom<=info.height);if(label==='iphone')assert.ok(info.summary.height<=78);}
