...
(async()=>{
 ...
 try{
  ...
  for(const [name,width,height,insets] of [['iphone-x',812,375,[0,44,21,44]],['iphone-small',568,320,[0,0,0,0]],['ipad',1024,768,[0,0,20,0]],['ipad-pro',1366,1024,[0,0,20,0]]]){
   ...
   await page.evaluate(insets=>['top','right','bottom','left'].forEach((side,i)=>document.documentElement.style.setProperty('--safe-'+side,insets[i]+'px')),insets);
   ...
   assert.ok(coverage.unchanged);for(const c of coverage.cases)assert.ok(c.left<=1e-6&&c.top<=1e-6&&c.right>=width-1e-6&&c.bottom>=height-1e-6,JSON.stringify(c));
   ...
   const rotated=await page.evaluate(()=>({a:__qa.screen({x:0,y:0}),b:__qa.screen({x:1672,y:941}),width:innerWidth,height:innerHeight}));
